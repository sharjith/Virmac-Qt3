...
class CMillCutter : public CGLObject  
{
    public:
	CMillCutter(const MillCutterType& typ	=   MILL_BALL_NOSE,
	...
	CBoundingBox GetAABB()  const { return  collisionAABB;  } 
	...
	bool operator == (CMillCutter) const;
	double m_dOX;
	double m_dOY;
	double m_dOZ;
	CListOfCTriangle* myTriangleList;
    private:
	void DrawWiredTool();
	void DrawShadedTool();
	void DrawHilitTool();
	...
	void AddSurfaceTriangles(CSurface* pSurf, const double& uParts, const double& vParts);
    private:
	GLint cutterList;
