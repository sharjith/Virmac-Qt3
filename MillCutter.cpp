...
CMillCutter::CMillCutter(const MillCutterType& typ,
...
CGLObject* CMillCutter::Copy() const
{
    CMillCutter* C = new CMillCutter(itsType, itsHolderDia, itsHolderLen, itsShankDia, itsShankLen, itsTaperDiaA, itsTaperDiaB, itsTaperLen, itsBodyDia, itsBodyLen, itsBallRad, bTip);
    ...
    C->MoveTo(CPoint3D(m_dOX, m_dOY, m_dOZ));
    return C;
}

void CMillCutter::SetCutterType  (const MillCutterType& typ)
{
    itsType = typ;
}

void CMillCutter::SetHolderDia  (const double& val)
{
    itsHolderDia = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetHolderLen  (const double& val)
{
    itsHolderLen = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetShankDia   (const double& val)
{
    itsShankDia = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetShankLen   (const double& val)
{
    itsShankLen = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetTaperDiaA  (const double& val)
{
    itsTaperDiaA = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetTaperDiaB  (const double& val)
{
    itsTaperDiaB = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetTaperLen   (const double& val)
{
    itsTaperLen = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetBodyDia	(const double& val)
{
    itsBodyDia = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetBodyLen	(const double& val)
{
    itsBodyLen = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetBallRad	(const double& val)
{
    itsBallRad = val;
    ComputeBoundLimits();
    BuildCollisionModel(0.02);
...
void CMillCutter::SetResolution( const ModelResolution& res)
{
    itsResolution = res;
    DefineDisplay();
}
...
#include "OneAxis.h"
void CMillCutter::MoveTo(const CPoint3D& P)
{
    CPoint3D p = P;

    p.Translate(m_dTransX,m_dTransY,m_dTransZ);

    p.Rotate(COneAxis(CPoint3D(m_dRotCX, m_dRotCY, m_dRotCZ), CVector3D(1,0,0)), m_dRotRX*PI/180.0);
    p.Rotate(COneAxis(CPoint3D(m_dRotCX, m_dRotCY, m_dRotCZ), CVector3D(0,1,0)), m_dRotRY*PI/180.0);
    p.Rotate(COneAxis(CPoint3D(m_dRotCX, m_dRotCY, m_dRotCZ), CVector3D(0,0,1)), m_dRotRZ*PI/180.0);
    ...
    for(itr.Init(); itr.More(); itr.Next())
    {
	myTriangleList->Append(itr.Current());
    }
    l->Clear();
    delete l;
    */
...
void CMillCutter::Rotate(double A, double B, double C)
{
    m_dRotA = A;
    m_dRotB = B;
    m_dRotC = C;
...
void CMillCutter::ComputeBoundLimits()
{
    double totlen = itsHolderLen + itsShankLen + itsTaperLen + itsBodyLen + itsBallRad + 120;
    itsBox.SetLimits(-55, 55, -55, 55, 0, totlen);
    collisionAABB.SetLimits(-MAX(itsBodyDia/2, itsHolderDia/2),
	    MAX(itsBodyDia/2, itsHolderDia/2),
	    -MAX(itsBodyDia/2, itsHolderDia/2),
	    MAX(itsBodyDia/2, itsHolderDia/2),
	    0, totlen);
}

void CMillCutter::SetTranslation(double X, double Y, double Z)
{
    m_dTransX = X;
    m_dTransY = Y;
    m_dTransZ = Z;
...
void CMillCutter::SetScale(double X, double Y, double Z)
{
    m_dScaleX = X;
    m_dScaleY = Y;
    m_dScaleZ = Z;
...
void CMillCutter::SetRotation(double CX, double CY, double CZ, double RX, double RY, double RZ)
{
    m_dRotCX = CX;
    m_dRotCY = CY;
    m_dRotCZ = CZ;
...
void CMillCutter::GetTranslation(double& X, double& Y, double& Z)
{
    X = m_dTransX;
    Y = m_dTransY;
    Z = m_dTransZ;
...
void CMillCutter::GetScale(double& X, double& Y, double& Z)
{
    X = m_dScaleX;
    Y = m_dScaleY;
    Z = m_dScaleZ;
...
void CMillCutter::GetRotation(double& CX, double& CY, double& CZ, double& RX, double& RY, double& RZ)
{
    CX = m_dRotCX;
    CY = m_dRotCY;
    CZ = m_dRotCZ;
...
bool CMillCutter::operator == (CMillCutter c) const
{
...
void CMillCutter::DefineDisplay()
{
    GLfloat bgcol[4];
...
void CMillCutter::Display(const GLDisplayMode& dMode)
{
    displayMode = dMode;
    glDepthFunc(GL_LESS);
    DefineDisplay();
...
void CMillCutter::Hilight(const GLDisplayMode& /*dMode*/)
{
    DrawHilitTool();
}

void CMillCutter::DoRotation()
{
...
void CMillCutter::DrawWiredTool()
{
...
void CMillCutter::DrawShadedTool()
{
    //if(displayMode == GLSHADED) 
    //	glColor4ub(itsShadeRed, itsShadeGreen, itsShadeBlue, 255);
    //else HLR
...
void CMillCutter::DrawHilitTool()
{
...
void CMillCutter::InitDisplayLists()
{
...
#include <Cone.h>
#include <Torus.h>
void CMillCutter::BuildCollisionModel(const double& chordalError)
{
...
void CMillCutter::AddSurfaceTriangles(CSurface* pSurf, const double& uParts, const double& vParts)
{
    CPoint3D curP1;
    double istep, jstep;
	...
	for(i = fUPar; i < lUPar; i += istep)
	{
	    ...
	    CTriangle T1(P2, P1, P3);
	    myTriangleList->Append(T1); ++numTriangles;
	    CTriangle T2(P1, P3, P4);
	    myTriangleList->Append(T2); ++numTriangles;
	}		
    }
}

void CMillCutter::RebuildDisplayLists()
{
    glDeleteLists(cutterList, 1);
    InitDisplayLists();
}

void CMillCutter::SetColor(const GLubyte& red, const GLubyte& green, const GLubyte& blue)
{
